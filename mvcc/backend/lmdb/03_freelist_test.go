...
func Test_freelist_free(t *testing.T) {
	...
	if !reflect.DeepEqual([]pgid{12}, f.pending[100]) {
		t.Fatalf("expected=%v; got=%v", []pgid{12}, f.pending[100])
	}
}

// Ensure that a page and its overflow is added to a transaction's freelist.
func Test_freelist_free_overflow(t *testing.T) {
	f := newFreelist()
	f.free(100, &page{id: 12, overflow: 3})
	if exp := []pgid{12, 13, 14, 15}; !reflect.DeepEqual(exp, f.pending[100]) {
		t.Fatalf("expected=%v; got=%v", exp, f.pending[100])
	}
}

// Ensure that a transaction's free pages can be released.
func Test_freelist_release(t *testing.T) {
	f := newFreelist()
	...
	if exp := []pgid{9, 12, 13}; !reflect.DeepEqual(exp, f.ids) {
		t.Fatalf("expected=%v; got=%v", exp, f.ids)
	}
...
func Test_freelist_read(t *testing.T) {
	// Create a page.
	var buf [4096]byte
...
func Test_freelist_write(t *testing.T) {
	...
	if exp := []pgid{3, 11, 12, 28, 39}; !reflect.DeepEqual(exp, f2.ids) {
		t.Fatalf("expected=%v; got=%v", exp, f2.ids)
	}
}
